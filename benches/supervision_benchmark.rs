use ash_flare::{
    ...
    StatefulSupervisorHandle, StatefulSupervisorSpec, Worker, WorkerContext,
};
...
impl Worker for StatefulWorker {
    ...
    async fn run(&mut self) -> Result<(), Self::Error> {
        ...
        self.context.set(format!("worker-{}", self.id), serde_json::json!(self.id));
            ...
            |b, &d| {
                b.to_async(tokio::runtime::Runtime::new().unwrap())
                    .iter(|| async move {
                        ...
                        let depth = d;
                        let mut spec = SupervisorSpec::new(format!("level-{}", depth))
                            .with_worker("worker", move || SimpleWorker { id: depth }, RestartPolicy::Temporary);
                        ...
                        for i in (0..d).rev() {
                            ...
                            spec = SupervisorSpec::new(name)
                                .with_worker("worker", move || SimpleWorker { id: worker_id }, RestartPolicy::Temporary)
                ...
                b.to_async(tokio::runtime::Runtime::new().unwrap())
                    .iter(|| async move {
                        ...
                        for i in 0..count {
                            spec = spec.with_worker(
                                format!("worker-{}", i),
                                move |ctx| StatefulWorker { id: i, context: ctx },
                                RestartPolicy::Temporary,
                            );
                        }
